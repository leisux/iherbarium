...
def edit_dng(dir):
    rdf = os_walk(dir)
    xmp_file = r"./Orientation8.xmp"
    for root, _, files in rdf:
        for file in files:
            name_ext = os_path_splitext(file)
            if name_ext[1].lower() == ".dng":
                dng_file = os_path_join(root, file)
                with exiftool.ExifTool(r"./exiftool.exe") as et:
                    et.execute("-overwrite_original", "-tagsfromfile", xmp_file, dng_file)
                extract_dng_preview(dng_file, root, name_ext[0])

def extract_dng_preview(file, root, name):
    with rawpy.imread(file) as raw:
        thumb = raw.extract_thumb()
    if thumb.format == rawpy.ThumbFormat.JPEG:
        with open(os_path_join(root, name+".jpg"), 'wb') as f:
            f.write(thumb.data)
    elif thumb.format == raw.py.ThumbFormat.BITMAP:
        imageio.imsave(os_path_join(root, name+".jpg"), thumb.data)
...
def rename(dir, width=3000):
    ...
    def ident(raw_img_path, width):
        ...
        if platform.system() == "Windows":
            ...
            zbarimg = '/opt/homebrew/bin/zbarimg'
        else:
            zbarimg = os_path_dirname(__file__) + "/./zbar/bin/zbarimg.exe"
        ...
        try:
            if " " in raw_img_path:
                ...
                x, y = im.size
                if x > width:
                    rm = im.resize((width, int(width*y/x)), Image.LANCZOS)
...
请选择和你 Windows 系统相匹配的 msi 文件安装\n\n")
                ...
                try:
                    ...
                    try:
                        raw_img_path = os_path_join(path, os_path_splitext(file)[0]+".DNG")
                        os_rename(raw_img_path, new_img_path+".DNG")
                    except:
                        pass
                cnt_ident += 1
        except KeyError:
...
if __name__ == '__main__':
    edit_dng(r"/Users/xuzhoufeng/Downloads/IBSC-6")