...
import _locale
import Kingdonia
from ipybd import BioGrid, Occurrence, CVH, Label
...
def main():
    ...
    while start == "y":
        ...
        【4】从 DNG 照片中提取 JPG 并重设照片指向\t\t【5】Excel 转 CVH 格式\n\n\
        【6】Excel 转标准表\t\t【7】Excel 转标签打印表\t\t【8】Excel 转 BioGrid\n\n")
        if opr == "1":
            print("\n请选择图片所在的文件夹，可以包括多层子文件夹，程序会自动忽略非图片文件\n")
            dir = askdir()
            if dir == "":
                continue
            width = input("\n请输入用于条码识别的照片预处理宽度（单位为像素，比如 3000）, 默认请按Enter：\n\n")
            if width == "":
                width = 3000
            else:
                width = int(width)
            Kingdonia.rename(dir, width)
        elif opr == "2":
        ...
        【3】中文植物通用标签（无条形码）\t\t【4】英文植物通用标签（无条形码）\n\n"))
            ...
            occ =BioGrid(excel)
            occ.rebuild_table()
            dir = os.path.splitext(excel)[0] + '_biogrid.xlsx'
            occ.save_data(dir)
        else:
            input("\n您的输入内容不匹配...\n\n")
